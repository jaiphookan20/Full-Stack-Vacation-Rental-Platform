...
const Perks = ( {selected, onChange} ) => {
    ...
    function handleCheckBoxClick(e) {
        ...
        if(checked == true) {
            ...
            // if a perk box has been checked, add it to the 
        }
        else {
            // eslint-disable-next-line react/prop-types
  ...
  return (
    <div className="grid mt-2 gap-2 sm:grid-cols-2 md:grid-cols-3 lg: grid-cols-2">
        <label className="border p-4 flex rounded-2xl gap-2 items-center pointer-cursor">
            
            {/* checked={selected.includes('wifi')} explanation: 
                The selected array is passed as a prop to the Perks component. It represents the currently selected perks.
                The includes('wifi') method is called on the selected array to check if the string 'wifi' exists in the array.
                If 'wifi' is present in the selected array, selected.includes('wifi') will return true, and the checked attribute will be set to true, which means the checkbox will be initially checked.
            */}
        ...
        <label className="border p-4 flex rounded-2xl gap-2 items-center pointer-cursor">
            <input name="entrance" checked={selected.includes('entrance')} type="checkbox" onChange={handleCheckBoxClick} />
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
